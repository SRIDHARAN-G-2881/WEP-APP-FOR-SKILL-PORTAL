...
export default function NightSkill() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:3000/api/nightskill", formData);
      if (res.status === 200) {
        console.log("The skill data saved successfully");
        setFormData({ name: "", dept: "", skill: "" }); // Clear form data
        setState(0); // Reset error state
        ...
        setState(1); // Handle unexpected status codes as errors
      }
    } catch (error) {
  ...
  return (
    <div className="p-5 max-w-4xl mx-auto flex flex-col justify-center items-center gap-6 bg-gradient-to-r from-gray-100 to-gray-300 rounded-lg shadow-lg">
      <h2 className="text-center text-4xl my-5 font-bold text-gray-800">Night Skill Choice Form</h2>
      <form className="flex flex-col gap-6 w-full max-w-lg bg-white p-8 rounded-lg shadow-md" onSubmit={handleSubmit}>
        ...
        <div className="w-full">
          <select
            id="dept"
            value={formData.dept} // Bind value to formData
        ...
        <div className="w-full">
          <select
            id="skill"
            value={formData.skill} // Bind value to formData
            className="w-full p-3 text-base border border-gray-300 rounded-lg bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500 transition duration-300"
            onChange={handleChange}
          >
        ...
        <div className="flex justify-center">
          <Button
            type="submit"
            className="font-semibold bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition duration-300"
          >
            Submit
          </Button>
        </div>
